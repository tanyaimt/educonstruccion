<?php
	...
	$aux=substr($btnAdministrarProyectoLogistica, strlen($btnAdministrarProyectoLogistica)-1, 1);
	$btnAdministrarProyectoLogistica= substr($btnAdministrarProyectoLogistica,0, strlen($btnAdministrarProyectoLogistica)-1);
	switch ($aux) {
		case 'a':
			$consulta="SELECT * FROM empresaCliente inner join empresa on idEmpresa=idEmpresa1 WHERE idEmpresaCliente='".$btnAdministrarProyectoLogistica."'";
		...
		break;
		case 'b':
			$consulta="SELECT * FROM obra WHERE idEmpresaCliente='".$idEmpresa."' and idObra='".$btnAdministrarProyectoLogistica."'";
			$resultado = mysqli_query($enlace,$consulta);
		...
		break;
		case 'c':
			$consulta="SELECT * FROM `contrato` WHERE idObra1='".$idObra."' and idContrato='".$btnAdministrarProyectoLogistica."'";
			$resultado = mysqli_query($enlace,$consulta);
			if (mysqli_num_rows($resultado) > 0) {
		    	while($row = mysqli_fetch_assoc($resultado)) {
		...
		break;
		case 'd':
			$consulta="SELECT * FROM cotizacion WHERE idContrato1='".$idContrato."' and idCotizacion='".$btnAdministrarProyectoLogistica."'";
			$resultado = mysqli_query($enlace,$consulta);
			if (mysqli_num_rows($resultado) > 0) {
		    	while($row = mysqli_fetch_assoc($resultado)) {
		...
		break;
		case 'e':
			$consulta="SELECT * FROM estimaciondestajo WHERE idCotizacion1='".$idCotizacion1."' and estimaciondestajo='".$btnAdministrarProyectoLogistica."'";
			$resultado = mysqli_query($enlace,$consulta);
			if (mysqli_num_rows($resultado) > 0) {
		    	while($row = mysqli_fetch_assoc($resultado)) {
