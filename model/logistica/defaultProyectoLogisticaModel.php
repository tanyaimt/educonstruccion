<?php
	...
	switch ($aux) {
		case 'a':
			$consulta="SELECT * FROM empresa WHERE idEmpresa='".$btnAdministrarProyectoLogistica."'";
		...
		break;
		case 'b':
			$consulta="SELECT * FROM obra WHERE idEmpresa2='".$idEmpresa."' and idObra='".$btnAdministrarProyectoLogistica."'";
			$resultado = mysqli_query($enlace,$consulta);
			$ConfirmarEmpresa="true";
			if (mysqli_num_rows($resultado) > 0) {
		    	while($row = mysqli_fetch_assoc($resultado)) {
		    		$idObra= $row["idObra"];
					$idEmpresa= $row["idEmpresa2"];
